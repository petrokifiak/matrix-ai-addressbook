...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_contact_lifecycle(self):
        # 1. Add contact
        res = execute_command(
            "add", ["John", "0501234567"], self.book, self.notebook
        )
        self.assertEqual(res, "Contact added.")

        # 2. Add second phone
        res = execute_command(
            "add-phone", ["John", "0671112233"], self.book, self.notebook
        )
        self.assertEqual(res, "Phone added.")

        # 3. Add email
        res = execute_command(
            "add-email", ["John", "john@example.com"], self.book, self.notebook
        )
        ...
        # 5. Add birthday
        res = execute_command(
            "add-birthday", ["John", "15.08.1995"], self.book, self.notebook
        )
        ...
        # 8. Change phone
        change_res = execute_command(
            "change",
            ["John", "0501234567", "0998887766"],
            self.book,
            self.notebook,
        )
        ...
        self.assertEqual(
            execute_command("phone", ["John"], self.book, self.notebook),
            "0998887766; 0671112233",
        )

        # 9. Delete contact
        del_res = execute_command(
            "delete-contact", ["John"], self.book, self.notebook
        )
    ...
    def test_full_note_lifecycle(self):
        ...
        # 2. Add tags
        res = execute_command(
            "add-tag", ["1", "food", "groceries"], self.book, self.notebook
        )
        self.assertEqual(res, "Tag added to note.")

        # 3. Search note by tag
        res = execute_command(
            "search-by-tag", ["food"], self.book, self.notebook
        )
        ...
        res = execute_command(
            "search-notes", ["Apples"], self.book, self.notebook
        )
        self.assertIn("Shopping", res)

        # 6. Sort notes by specific tags
        res_sort = execute_command(
            "sort-notes-by-tags",
            ["food", "groceries"],
            self.book,
            self.notebook,
        ...
        # 7. Delete note
        res = execute_command(
            "delete-note", ["1"], self.book, self.notebook
        )
    ...
    def test_persistence_save_and_load(self):
        ...
        try:
            ...
            execute_command(
                "add", ["Alice", "0501112233"], self.book, self.notebook
            )
            execute_command(
                "add-email",
            ...
            self.assertEqual(
                loaded_book.data["Alice"].phones[0].value, "0501112233"
            )
            self.assertEqual(
                loaded_book.data["Alice"].emails[0].value, "alice@example.com"
            )
            ...
            self.assertEqual(
                loaded_note.content, "Discuss project roadmap"
            )
        finally:
